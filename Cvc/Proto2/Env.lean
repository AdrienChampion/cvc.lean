...
A fork of `Cvc/Basic/Env.lean` whose state carries a **registry of declared sorts** alongside the
term manager. That is what makes a datatype nameable by `ToTyp`: see `State`.
