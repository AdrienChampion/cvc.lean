...
gen_untyped% from Cvc.Proto2.Spec.Float
...
def toTerm : (f : Cvc.Proto2.Float exp sig) → Env Term
...
def ofTermErased (term : Term) : Env Cvc.Proto2.Float.Erased := do
  ...
  let mk (f : Cvc.Proto2.Float exp sig) : Env Cvc.Proto2.Float.Erased := return {exp, sig, get := f}
...
def ofTerm (term : Term) : Env (Cvc.Proto2.Float exp sig) := do
