...
namespace Cvc.Proto2.Untyped.Term public section

open cvc5 renaming Term → T

variable [Ω]

gen_untyped% from Cvc.Proto2.Spec.FiniteField
...
def toTerm (elem : Cvc.Proto2.FiniteField size) : Env Term := do
  mkFiniteFieldElem elem.repr (← Srt.of (Cvc.Proto2.FiniteField size))

/-- The finite-field element a constant term denotes, at whatever size it turns out to have. -/
def ofTermErased (term : Term) : Env Cvc.Proto2.FiniteField.Erased := do
...
def ofTerm (term : Term) : Env (Cvc.Proto2.FiniteField size) := do
