...
gen_untyped% from Cvc.Proto2.Spec.Array
...
def mkArrayValue [ValueToTerm α] [ValueToTerm β] (map : Cvc.Proto2.TotalMap α β) : Env Term := do
  let idx ← Srt.of α
...
: Env (Cvc.Proto2.TotalMap α β) :=
  peel Std.TreeMap.empty term
where
  peel (acc : Std.TreeMap α β) (term : Term) : Env (Cvc.Proto2.TotalMap α β) := do
