...
public import Cvc.Proto2.Untyped.Term.Float
...
gen_typed% from Cvc.Proto2.Spec.Float
...
def isFloatValue (term : Term (Cvc.Proto2.Float exp sig)) : Bool := T.isFloatValue term
@[inherit_doc T.isFloatNaN]
def isFloatNaN (term : Term (Cvc.Proto2.Float exp sig)) : Bool := T.isFloatNaN term
@[inherit_doc T.isFloatPosInf]
def isFloatPosInf (term : Term (Cvc.Proto2.Float exp sig)) : Bool := T.isFloatPosInf term
@[inherit_doc T.isFloatNegInf]
def isFloatNegInf (term : Term (Cvc.Proto2.Float exp sig)) : Bool := T.isFloatNegInf term
@[inherit_doc T.isFloatPosZero]
def isFloatPosZero (term : Term (Cvc.Proto2.Float exp sig)) : Bool := T.isFloatPosZero term
@[inherit_doc T.isFloatNegZero]
def isFloatNegZero (term : Term (Cvc.Proto2.Float exp sig)) : Bool := T.isFloatNegZero term
...
-/
def getFloatComponents (term : Term (Cvc.Proto2.Float exp sig))
...
def mkFloatValue
: (f : Cvc.Proto2.Float exp sig) → Env (Term (Cvc.Proto2.Float exp sig)) := T.Float.toTerm

/-- The floating-point value a constant term denotes. -/
def getFloatValue : Term (Cvc.Proto2.Float exp sig) → Env (Cvc.Proto2.Float exp sig) := T.Float.ofTerm

instance : SrtLike (Cvc.Proto2.Float exp sig) where
