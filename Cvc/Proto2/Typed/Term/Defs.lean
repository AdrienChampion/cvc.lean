...
`Cvc.Proto2.Typed.Term` names both this type and the namespace holding the generated constructors,
so a signature can mention `Term α` without reaching outside the layer.
-/
namespace Cvc.Proto2.Typed public section variable [Ω]

wrap3% Term α ← Cvc.Proto2.Untyped.Term (ofUntypedUnsafe / toUntypedUnsafe)
...
def _root_.Cvc.Proto2.Untyped.Term.typeCheckAs (t : Untyped.Term) (α : Type) [ToTyp α]
: Env (Term α) := Typed.Term.ofUntyped' t α

@[inherit_doc Untyped.Term.typeCheckAs]
def _root_.Cvc.Proto2.Untyped.Term.typeCheck [ToTyp α] (t : Untyped.Term)
...
`scoped`, so that `open Cvc.Proto2.Typed` selects this layer's DSL. It forwards to `smtT!`, which
expands to fully-qualified `Cvc.Proto2.Typed.Term.…` calls; nothing needs to be in scope.
