...
form and registered in `Cvc.Proto2.opSpecExt`; nothing is generated at the declaration site.
...
def elabOpStx : CommandElab
  | `(
    ...
    op% $id $[ [ $indexIds* ] ]? $[ { $binderIds* $[ : $binderClasses?,* ]? } ]*
      ...
      : $ret := $kind $[ $build?:protoBuild ]? $[ $notation?:protoNotation ]?
        $[ $nary?:protoNary ]?
  ) => do
    ...
    let nary : Nary ← match nary? with
      ...
      | some stx => match stx with
        | `(protoNary| nary) => pure Nary.array
        | `(protoNary| naryAs $nm:ident) => do
          naryName := some nm.getId
          pure Nary.array
        | `(protoNary| unit $u) => Nary.arrayWithUnit <$> toNaryUnit u
    ...
    let tmFn : Option Name ← match build? with
      | none => pure none
      | some stx => match stx with
        | `(protoBuild| tm $fn:ident) => pure (some fn.getId)
        | `(protoBuild| tmSortOf $fn:ident $shape) => do
    ...
    let smtNotation : Notation ← match notation? with
      ...
      | some stx => match stx with
        | `(protoNotation| infixl $s:str $p:num) =>
          pure (Notation.inf s.getString p.getNat .left)
        | `(protoNotation| infixr $s:str $p:num) =>
          pure (Notation.inf s.getString p.getNat .right)
        | `(protoNotation| prefix $s:str $p:num $a:num) =>
