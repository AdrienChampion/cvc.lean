...
public meta import Cvc.Proto2.Types.Float
...
-/
namespace Cvc.Proto2.Tests.Fun
...
#guard_msgs in #eval Env.runIO do
  ...
  println! "sets and bags    : {← Srt.of ((Cvc.Proto2.Set Int) → (Cvc.Proto2.Bag Int))}"
  println! "array domain     : {← Srt.of ((Cvc.Proto2.TotalMap Int Int) → Bool)}"
  println! "bit-vec and seq  : {← Srt.of ((BitVec 4) → (Array Int))}"
  println! "string and regex : {← Srt.of (String → Cvc.Proto2.Regex)}"
  println! "float and mode   : {← Srt.of ((Cvc.Proto2.Float 8 24) → Cvc.Float.RoundingMode)}"
...
#guard_msgs in #check @Cvc.Proto2.Untyped.Term.apply2
...
#guard_msgs in #check @Cvc.Proto2.Typed.Term.setMap
...
variable [Ω]
  ...
  (s : Term (Cvc.Proto2.Set Int)) (bg : Term (Cvc.Proto2.Bag Int))
  (z : Term Rat)

/-- Mapping rewrites the element index. -/
example : Env (Term (Cvc.Proto2.Set Int)) := setMap m s

/-- Filtering preserves it. -/
example : Env (Term (Cvc.Proto2.Set Int)) := setFilter p s
...
example (eq : Term (Int → Int → Bool)) : Env (Term (Cvc.Proto2.Bag (Cvc.Proto2.Bag Int))) :=
  bagPartition eq bg
example : Env (Term (Cvc.Proto2.Bag Int)) := bagFilter p bg
