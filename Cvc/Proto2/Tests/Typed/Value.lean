...
-/
namespace Cvc.Proto2.Tests.Typed.Value
...
#guard_msgs in #check @Cvc.Proto2.Typed.Term.getValue

/-- info: @Term.getBitVecValue : [inst : Ω] → {size : Nat} → Term (BitVec size) → Env (BitVec size) -/
#guard_msgs in #check @Cvc.Proto2.Typed.Term.getBitVecValue
...
variable [Ω] (i : Term Int) (r : Term Rat) (bv : Term (BitVec 8))
  (rm : Term Cvc.Float.RoundingMode) (f : Term (Cvc.Proto2.Float 8 24))
...
#guard_msgs in #check @Cvc.Proto2.Typed.Term.isInt32Value

/-- info: @Term.getSign : [inst : Ω] → {α : Type} → [inst_1 : ToTyp α] → [IsArith α] → Term α → Res Int -/
#guard_msgs in #check @Cvc.Proto2.Typed.Term.getSign
...
#guard_msgs in #check @Cvc.Proto2.Typed.Term.getKids
