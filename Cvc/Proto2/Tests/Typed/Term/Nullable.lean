...
-/
namespace Cvc.Proto2.Tests.Typed.Term.Nullable
...
#guard_msgs in #check @Cvc.Proto2.Typed.Term.nullableVal
...
-/
#guard_msgs in #eval Env.runIO do
  let roundTrip (α : Type) [Cvc.Proto2.Typed.SrtLike α] (value : Option α) [ToString α]
  : Env String := do
...
#guard_msgs in #eval Env.runIO do
  ...
  let set ← s.declareConst (Cvc.Proto2.Set (Option Int)) "set"
  (do setMember x set) >>= s.assert

  s.checkSat (ifSat := do
    println! "x     : {← s.getValue x}"
    println! "y     : {← s.getValue y}"
    let elems : Cvc.Proto2.Set (Option Int) ← s.getValue set
