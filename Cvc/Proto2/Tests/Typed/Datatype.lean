...
a description. This fork keeps each declared sort in the scope's registry under its name, and
...
-/
namespace Cvc.Proto2.Tests.Typed.Datatype
...
#guard_msgs in #eval Env.runIO do
  ...
  println! "Set Pair      : {← (Srt.of (Proto2.Set Pair) : Env Srt)}"
...
#guard_msgs in #check @Cvc.Proto2.Typed.Datatype.Field.get

/-- info: @Datatype.Ctor.is : {α : Type} → [inst : Ω] → Datatype.Ctor α → Term α → Env (Term Bool) -/
#guard_msgs in #check @Cvc.Proto2.Typed.Datatype.Ctor.is
...
#guard_msgs in #check @Cvc.Proto2.Typed.Term.mkMatch
