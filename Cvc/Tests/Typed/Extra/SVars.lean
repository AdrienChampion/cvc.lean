...
public meta import Cvc.Typed.Extra.Sys
...
-/
namespace Cvc.Tests.Typed.SVars
...
example (s : Counter.TermsAt 0) : Term Int := s.count
...
example {ι : SVars Counter} (s : ι.TermsAt 0) : Env (Term Int) := smt! s.count + 1
example {ι : SVars Counter} (s : ι.ValuesAt 0) : Bool := s.counting

/-! ## Symbols are named per step

`declareAt` names a symbol `count__@__k`, so a step's variables are distinguishable by name and a
model can say which step a value belongs to. Both layers share the convention, and the sort-erased
`identAt` is what the typed one names it through.
-/
