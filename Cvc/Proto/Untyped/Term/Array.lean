...
gen_untyped% from Cvc.Proto.Spec.Array
...
def mkArrayValue [ValueToTerm α] [ValueToTerm β] (map : Cvc.Proto.TotalMap α β) : Env Term := do
  let srt ← Srt.of (Cvc.Proto.TotalMap α β)
  let mut term ← mkValue map.defaultElem >>= mkConstArray srt
...
: Env (Cvc.Proto.TotalMap α β) :=
  peel Std.TreeMap.empty term
where
  peel (acc : Std.TreeMap α β) (term : Term) : Env (Cvc.Proto.TotalMap α β) := do
