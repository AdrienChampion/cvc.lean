...
gen_untyped% from Cvc.Proto.Spec.Float
...
def mkRoundingMode (rm : Cvc.Float.RoundingMode) : Env Term :=
  Cvc.runUnsafe fun tm => tm.mkRoundingMode rm.toUnsafe
...
def toTerm : (f : Cvc.Proto.Float exp sig) → Env Term
...
def ofTermErased (term : Term) : Env Cvc.Proto.Float.Erased := do
  ...
  let mk (f : Cvc.Proto.Float exp sig) : Env Cvc.Proto.Float.Erased := return {exp, sig, get := f}
...
def ofTerm (term : Term) : Env (Cvc.Proto.Float exp sig) := do
