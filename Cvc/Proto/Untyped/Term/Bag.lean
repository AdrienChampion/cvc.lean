...
public import Cvc.Proto.Untyped.Term.Arith
...
gen_untyped% from Cvc.Proto.Spec.Bag



/-! ## Values

A `Cvc.Proto.Bag` denotes an SMT bag: the empty bag, unioned with one singleton bag per element, each
...
def mkBagValue [ValueToTerm α] (bag : Cvc.Proto.Bag α) : Env Term := do
  let empty ← Srt.of (Cvc.Proto.Bag α) >>= bagEmpty
...
-/
partial def getBagValue [TermToValue α] (term : Term) : Env (Cvc.Proto.Bag α) := do
  let kind ← term.getKind
  match kind with
  | .BAG_EMPTY => return Cvc.Proto.Bag.empty
  | .BAG_MAKE =>
    let ⟨kids, _⟩ ← term.getSizedKids 2
    return Cvc.Proto.Bag.empty.insert (← getValue kids[0]) (← getValue kids[1])
  | .BAG_UNION_MAX | .BAG_UNION_DISJOINT =>
    let ⟨kids, _⟩ ← term.getSizedKids 2
    let lft : Cvc.Proto.Bag α ← getBagValue kids[0]
    let rgt : Cvc.Proto.Bag α ← getBagValue kids[1]
