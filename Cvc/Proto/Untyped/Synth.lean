...
import all Cvc.Proto.Untyped.BVar
import all Cvc.Proto.Untyped.Solver

public import Cvc.Basic
public import Cvc.Proto.Untyped.Mode
...
@[inherit_doc cvc5.SynthResult.isNull]
def isNull : Bool := r.toUnsafe.isNull
...
def declareSygusVar (symbol : String) (srt : Srt) : Env Term :=
  Cvc.runUnsafe' do s.toUnsafe.declareSygusVar symbol srt.toUnsafe
...
def addSygusInvConstraint (inv pre trans post : Term) : Env Unit :=
  Cvc.runUnsafe' do s.toUnsafe.addSygusInvConstraint inv pre trans post
...
private def constraintsForError : Env (Array String) := do
  let constraints ← Cvc.runUnsafe' do s.toUnsafe.getSygusConstraints
...
def checkSynthResult : Env SynthResult :=
  SynthResult.ofUnsafe <$> Cvc.runUnsafe' s.toUnsafe.checkSynth

/-- Tries to find *another* solution, as a raw result. -/
def checkSynthNextResult : Env SynthResult :=
  SynthResult.ofUnsafe <$> Cvc.runUnsafe' s.toUnsafe.checkSynthNext
...
def getSynthSolution (fn : Term) : EnvSolved Term :=
  Cvc.runUnsafe' do s.toUnsafe.getSynthSolution fn

@[inherit_doc getSynthSolution]
def getSynthSolutions (fns : Terms) : EnvSolved Terms :=
  Cvc.runUnsafe' do s.toUnsafe.getSynthSolutions fns
...
def findSynth (target : Cvc.Synth.FindTarget) (grammar : Option Grammar := none) : Env Term :=
  Cvc.runUnsafe' do s.toUnsafe.findSynth target.toUnsafe grammar

@[inherit_doc findSynth]
def findSynthNext : Env Term :=
  Cvc.runUnsafe' do s.toUnsafe.findSynthNext
