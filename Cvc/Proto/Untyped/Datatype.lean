...
def mk (name : String) : Env Datatype.Constructor.Decl :=
  Cvc.runUnsafe fun tm => tm.mkDatatypeConstructorDecl name
...
def addSelector (name : String) (srt : Srt) : Env Datatype.Constructor.Decl :=
  Cvc.runUnsafe' do d.toUnsafe.addSelector name srt.toUnsafe
...
def addSelectorSelf (name : String) : Env Datatype.Constructor.Decl :=
  Cvc.runUnsafe' do d.toUnsafe.addSelectorSelf name
...
: Env Datatype.Constructor.Decl :=
  Cvc.runUnsafe' do d.toUnsafe.addSelectorUnresolved name unresDatatypeName

@[inherit_doc DtCD.isNull]
def isNull : Bool := d.toUnsafe.isNull
...
def mk (name : String) (params : Srts := #[]) (isCoDatatype : Bool := false) : Env Datatype.Decl :=
  Cvc.runUnsafe fun tm => tm.mkDatatypeDecl name params isCoDatatype
...
def addConstructor (ctor : Datatype.Constructor.Decl) : Env Datatype.Decl :=
  Cvc.runUnsafe' do d.toUnsafe.addConstructor ctor.toUnsafe
...
def isResolved : Env Bool := Cvc.runUnsafe' d.toUnsafe.isResolved

@[inherit_doc DtD.isNull]
def isNull : Bool := d.toUnsafe.isNull
...
: Env Srt :=
  Cvc.runUnsafe' do s.toUnsafe.declareDatatype symbol (ctors.map Datatype.Constructor.Decl.toUnsafe)
...
def getTerm : Env Term := Cvc.runUnsafe' do s.toUnsafe.getTerm

/-- The updater term, the first argument of an `applyUpdater`. -/
def getUpdaterTerm : Env Term := Cvc.runUnsafe' do s.toUnsafe.getUpdaterTerm

@[inherit_doc DtS.getCodomainSort]
def getCodomainSort : Env Srt := Cvc.runUnsafe' do s.toUnsafe.getCodomainSort

@[inherit_doc DtS.isNull]
def isNull : Bool := s.toUnsafe.isNull
...
def getTerm : Env Term := Cvc.runUnsafe' do c.toUnsafe.getTerm

/-- The constructor term of a *parametric* datatype, at the given instance of it. -/
def getInstantiatedTerm (retSort : Srt) : Env Term :=
  Cvc.runUnsafe' do c.toUnsafe.getInstantiatedTerm retSort.toUnsafe

/-- The tester term, the first argument of an `applyTester`. -/
def getTesterTerm : Env Term := Cvc.runUnsafe' do c.toUnsafe.getTesterTerm
...
def getSelector (name : String) : Env Datatype.Selector :=
  Cvc.runUnsafe' do c.toUnsafe.getSelector name
...
@[inherit_doc DtC.isNull]
def isNull : Bool := c.toUnsafe.isNull
...
def getConstructor (name : String) : Env Datatype.Constructor :=
  Cvc.runUnsafe' do dt.toUnsafe.getConstructor name
...
def getSelector (name : String) : Env Datatype.Selector :=
  Cvc.runUnsafe' do dt.toUnsafe.getSelector name

@[inherit_doc Dt.getParameters]
def getParameters : Env Srts := Cvc.runUnsafe' do dt.toUnsafe.getParameters
...
@[inherit_doc Dt.isNull]
def isNull : Bool := dt.toUnsafe.isNull
...
def applyConstructor (ctor : Term) (args : Terms := #[]) : Env Term :=
  Cvc.runUnsafe fun tm => tm.mkTerm .APPLY_CONSTRUCTOR (#[ctor] ++ args)
...
def applySelector (sel : Term) (dt : Term) : Env Term :=
  Cvc.runUnsafe fun tm => tm.mkTerm .APPLY_SELECTOR #[sel, dt]

/-- Whether a datatype value was built by a given constructor. -/
def applyTester (tester : Term) (dt : Term) : Env Term :=
  Cvc.runUnsafe fun tm => tm.mkTerm .APPLY_TESTER #[tester, dt]
