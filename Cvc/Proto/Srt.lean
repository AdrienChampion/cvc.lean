...
*fresh* sort even from an identical declaration — see `Cvc/Proto/Untyped/Datatype.lean`.
-/

@[inherit_doc Tm.mkDatatypeSort]
def datatype (decl : Datatype.Decl) : Env Srt := lift% mkDatatypeSort decl
...
-/
def datatypes (decls : Array Datatype.Decl) : Env Srts :=
  runUnsafe fun tm => tm.mkDatatypeSorts decls
...
| bag _ | set _ | seq _ | prod _
| abstract _ | function _ _ => false
