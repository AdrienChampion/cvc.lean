...
public meta import Cvc.Proto.Types.Float
...
#guard_msgs in #eval Env.runIO do
  ...
  println! "sets and bags    : {← Srt.of ((Cvc.Proto.Set Int) → (Cvc.Proto.Bag Int))}"
  println! "array domain     : {← Srt.of ((Cvc.Proto.TotalMap Int Int) → Bool)}"
  println! "bit-vec and seq  : {← Srt.of ((BitVec 4) → (Array Int))}"
  println! "string and regex : {← Srt.of (String → Cvc.Proto.Regex)}"
  println! "float and mode   : {← Srt.of ((Cvc.Proto.Float 8 24) → Cvc.Float.RoundingMode)}"
...
#guard_msgs in #check @Cvc.Proto.Untyped.Term.apply2
...
#guard_msgs in #check @Cvc.Proto.Typed.Term.setMap
...
variable [Ω]
  ...
  (s : Term (Cvc.Proto.Set Int)) (bg : Term (Cvc.Proto.Bag Int))
  (z : Term Rat)

/-- Mapping rewrites the element index. -/
example : Env (Term (Cvc.Proto.Set Int)) := setMap m s

/-- Filtering preserves it. -/
example : Env (Term (Cvc.Proto.Set Int)) := setFilter p s
...
example (eq : Term (Int → Int → Bool)) : Env (Term (Cvc.Proto.Bag (Cvc.Proto.Bag Int))) :=
  bagPartition eq bg
example : Env (Term (Cvc.Proto.Bag Int)) := bagFilter p bg
