...
-/
namespace Cvc.Proto.Tests.Untyped.Value
...
#guard_msgs in #eval Env.runIO do
  ...
  println! "Set   : {← roundTrip (Cvc.Proto.Set Int) (Cvc.Proto.Set.ofArray #[1, 2, 3])}"
...
#guard_msgs in #eval Env.runIO do
  ...
  println! "Set   : {← roundTrip (Cvc.Proto.Set Int) Cvc.Proto.Set.empty}"
...
-/
#guard_msgs in #eval Env.runIO do
  let bag : Cvc.Proto.Bag Int := (Cvc.Proto.Bag.empty.insert 1 2).insert 3 1
  println! "Bag      : {← Term.getValueAs (Cvc.Proto.Bag Int) (← Term.mkValue bag)}"
  let empty : Cvc.Proto.Bag Int := Cvc.Proto.Bag.empty
  println! "empty bag: {← Term.getValueAs (Cvc.Proto.Bag Int) (← Term.mkValue empty)}"

  let map : Cvc.Proto.TotalMap Int Int := ((Cvc.Proto.TotalMap.mkConst 0).insert 1 10).insert 2 20
  println! "TotalMap : {← Term.getValueAs (Cvc.Proto.TotalMap Int Int) (← Term.mkValue map)}"
...
`Cvc.Proto.Float exp sig` follows SMT-LIB: `sig` counts the hidden bit, so the pattern is
...
-/
#guard_msgs in #eval Env.runIO do
  let roundTrip (f : Cvc.Proto.Float 8 24) : Env (Cvc.Proto.Float 8 24) := do
    Term.getValueAs (Cvc.Proto.Float 8 24) (← Term.mkValue f)
  ...
  let outcome ←
    try
      let single ← Term.mkValueAs (Cvc.Proto.Float 8 24) .posInf
      let _ ← Term.getValueAs (Cvc.Proto.Float 11 53) single
      pure "no error"
    catch _ => pure "caught"
...
-/
#guard_msgs in #eval Env.runIO do
  let elem : Cvc.Proto.FiniteField 5 := {repr := 3}
  let term ← Term.mkValue elem
  println! "field      : {← Term.getValueAs (Cvc.Proto.FiniteField 5) term}"

  let outcome ←
    try
      let _ ← Term.getValueAs (Cvc.Proto.FiniteField 7) term
      pure "no error"
    catch _ => pure "caught"
  println! "wrong size : {outcome}"
...
#guard_msgs in #eval Env.runIO do
  ...
  println! "built set: {(← Term.mkValue (Cvc.Proto.Set.ofArray #[(1 : Int)])).isSetValue}"
  println! "seq      : {(← Term.mkValueAs (Array Int) #[]).isSeqValue}"
  let arr ← Term.mkConstArray (← Srt.of (Cvc.Proto.TotalMap Int Int)) (← Term.mkInt 0)
  println! "const arr: {arr.isConstArray}"
  let ff ← Term.mkValueAs (Cvc.Proto.FiniteField 5) {repr := 3}
...
#guard_msgs in #eval Env.runIO do
  ...
  let posInf ← Term.mkValueAs (Cvc.Proto.Float 8 24) .posInf
...
negZero {posInf.isFloatNegZero}"
  ...
  let nan ← Term.mkValueAs (Cvc.Proto.Float 8 24) .nan
...
-- identity: every term has an id, and none built here is the null term
/-- info: distinct ids : true, null : false -/
#guard_msgs in #eval Env.runIO do
  let x ← Term.mkInt 1
  let y ← Term.mkInt 2
  println! "distinct ids : {(← x.getId) != (← y.getId)}, null : {x.isNull}"
