...
import Cvc.Proto.Untyped.Grammar
import Cvc.Proto.Untyped.Term

public meta import Cvc.Proto.Untyped.Grammar
public meta import Cvc.Proto.Untyped.Term



/-! # SyGuS grammars, sort-erased -/
namespace Cvc.Proto.Tests.Untyped.Grammar
...
#guard_msgs in #eval Env.runIO do
  ...
  let caught (code : Env Cvc.Proto.Grammar) : Env String :=
...
example : Env Cvc.Proto.Grammar := s.mkGrammar #[] start

/-- Auxiliary non-terminals are separate, and cannot be mistaken for the start symbol. -/
example : Env Cvc.Proto.Grammar := s.mkGrammar #[] start #[cnd]

/-- info: @Solver.mkGrammar : [inst : Ω] → Solver → BVars → NT → optParam NTs #[] → Env Grammar -/
#guard_msgs in #check @Cvc.Proto.Untyped.Solver.mkGrammar
