...
-/
namespace Cvc.Proto.Tests.Typed.Term.Fun
...
#guard_msgs in #eval Env.runIO do
  let f ← Cvc.Proto.Typed.Term.mkSymbolAs (Int → Bool) "f"
  let g ← Cvc.Proto.Typed.Term.mkSymbolAs (Int → Bool → Rat) "g"
  let h ← Cvc.Proto.Typed.Term.mkSymbolAs (Int → Bool → Rat → Int) "h"
  let hof ← Cvc.Proto.Typed.Term.mkSymbolAs ((Int → Bool) → Rat) "hof"
...
/-- info: f(1) = 2 -/
#guard_msgs in #eval Env.runIO do
  let solver ← Cvc.Proto.Typed.Solver.new
  solver.setOption "produce-models" "true"

  let f ← Cvc.Proto.Typed.Term.mkSymbolAs (Int → Int) "f"
  let one ← Cvc.Proto.Typed.Term.mkInt 1
  let two ← Cvc.Proto.Typed.Term.mkInt 2
  let three ← Cvc.Proto.Typed.Term.mkInt 3
...
#guard_msgs in #eval Env.runIO do
  ...
  let g ← Cvc.Proto.Typed.Term.mkSymbolAs (Int → Int → Int) "g"
  let one ← Cvc.Proto.Typed.Term.mkInt 1
  let two ← Cvc.Proto.Typed.Term.mkInt 2
  let five ← Cvc.Proto.Typed.Term.mkInt 5
...
#guard_msgs in #eval Env.runIO do
  ...
  let f ← Cvc.Proto.Typed.Term.mkSymbolAs (Int → Int) "f"
  let x ← Cvc.Proto.Typed.Term.mkSymbolAs Int "x"
  let one ← Cvc.Proto.Typed.Term.mkInt 1
  let two ← Cvc.Proto.Typed.Term.mkInt 2
...
#guard_msgs in #eval Env.runIO do
  ...
  let f ← Cvc.Proto.Typed.Term.mkSymbolAs (Int → Int) "f"
  let x ← Cvc.Proto.Typed.Term.mkSymbolAs Int "x"
  let y ← Cvc.Proto.Typed.Term.mkSymbolAs Int "y"
