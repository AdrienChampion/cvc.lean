...
-/
namespace Cvc.Proto.Tests.Typed.Value
...
#guard_msgs in #check @Cvc.Proto.Typed.Term.getValue

/-- info: @Term.getBitVecValue : [inst : Ω] → {size : Nat} → Term (BitVec size) → Env (BitVec size) -/
#guard_msgs in #check @Cvc.Proto.Typed.Term.getBitVecValue
...
variable [Ω] (i : Term Int) (r : Term Rat) (bv : Term (BitVec 8))
  (rm : Term Cvc.Float.RoundingMode) (f : Term (Cvc.Proto.Float 8 24))
...
#guard_msgs in #check @Cvc.Proto.Typed.Term.isInt32Value

/-- info: @Term.getSign : [inst : Ω] → {α : Type} → [inst_1 : ToTyp α] → [IsArith α] → Term α → Res Int -/
#guard_msgs in #check @Cvc.Proto.Typed.Term.getSign
...
#guard_msgs in #check @Cvc.Proto.Typed.Term.getKids
