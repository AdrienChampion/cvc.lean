...
`wrap0%`–`wrap3%` declare a `def` equal to a cvc5 type, with `private` conversions either way, so
...
*constructors* are in `Cvc/Proto/Srt.lean`; only the types are here, so that the macros and the
...
declare_syntax_cat protoWrapKw

syntax "wrap0% " : protoWrapKw
syntax "wrap1% " : protoWrapKw
syntax "wrap2% " : protoWrapKw
syntax "wrap3% " : protoWrapKw

syntax (name := protoWrap)
  protoWrapKw ident (ident)? " ← " ident
    ("( " ident " / " ident " )")?
: command

@[command_elab protoWrap]
def elab_protoWrap : CommandElab
| `( $kw:protoWrapKw $id $[ $tParam? ]? ← $subId $[ ( $ofSubId? / $toSubId? ) ]? ) => do
  let tg ← TypeGen.mk id subId tParam? ofSubId? toSubId?
  match kw with
  | `(protoWrapKw| wrap0%) => tg.elab0
  | `(protoWrapKw| wrap1%) => tg.elab1
  | `(protoWrapKw| wrap2%) => tg.elab2
  | `(protoWrapKw| wrap3%) => tg.elab3
...
wrap0% Proof ← cvc5.Proof

wrap1% Command ← cvc5.Command

wrap2% Datatype.Decl ← cvc5.DatatypeDecl
wrap2% Datatype ← cvc5.Datatype
wrap2% Datatype.Constructor.Decl ← cvc5.DatatypeConstructorDecl
wrap2% Datatype.Constructor ← cvc5.DatatypeConstructor
wrap2% Datatype.Selector ← cvc5.DatatypeSelector
wrap2% Grammar ← cvc5.Grammar
...
protected def beq : Typ → Typ → Bool
  ...
  | bag e₁, bag e₂ | set e₁, set e₂ | seq e₁, seq e₂ => Typ.beq e₁ e₂
...
protected theorem beq_refl : (t : Typ) → Typ.beq t t = true
  ...
  | bitVec _ | finiteField _ | abstract _ => by simp [Typ.beq]
  | float _ _ => by simp [Typ.beq]
  | arrayTo i e => by simp [Typ.beq, Typ.beq_refl i, Typ.beq_refl e]
  | bag e | set e | seq e => by simp [Typ.beq, Typ.beq_refl e]
...
protected theorem eq_of_beq : (t₁ t₂ : Typ) → Typ.beq t₁ t₂ = true → t₁ = t₂
  ...
  | bitVec _, t₂, h | finiteField _, t₂, h | abstract _, t₂, h => by
