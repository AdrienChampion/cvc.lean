...
/-- Throws an `Error.internal` with an unsupported-feature message. -/
def throwTodo : m α := do
  throw <| Error.internal s!"[unsupported] {msg}"
...
export Error (throwUser throwInternal throwTodo throwUnreachable throwIO)
